...
/**
 * Auto-generated ApiCredential Routes
 *
...
import type { ApiCredentialHooks } from "./api-credential.hooks"
...
/**
 * Set hooks for ApiCredential routes
 */
export function setApiCredentialHooks(h: ApiCredentialHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "apiKey", "secretKey", "permissions", "isActive", "lastUsedAt", "createdAt"] as const
...
/**
 * Create ApiCredential CRUD routes
 */
export function createApiCredentialRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ApiCredential] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.apiCredential.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ApiCredential not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ApiCredential] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.apiCredential.create({
        data: picked.data,
      })
      ...
      console.error("[ApiCredential] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.apiCredential.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ApiCredential] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ApiCredential] Delete error:", error)
