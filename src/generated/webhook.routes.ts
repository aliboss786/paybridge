...
import type { WebhookHooks } from "./webhook.hooks"
...
/**
 * Set hooks for Webhook routes
 */
export function setWebhookHooks(h: WebhookHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "url", "events", "secret", "isActive", "lastTriggered", "createdAt"] as const
...
export function createWebhookRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.webhook.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Webhook not found" } }, 404)
      }
