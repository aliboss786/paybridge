...
import type { LedgerHooks } from "./ledger.hooks"
...
/**
 * Set hooks for Ledger routes
 */
export function setLedgerHooks(h: LedgerHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "transactionId", "type", "amount", "balanceAfter", "description", "createdAt"] as const
...
/**
 * Create Ledger CRUD routes
 */
export function createLedgerRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.ledger.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Ledger not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.ledger.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.ledger.update({
        where: { id },
        data: picked.data,
      })
