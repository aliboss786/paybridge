...
/**
 * Auto-generated TransactionEvent Routes
 *
...
import type { TransactionEventHooks } from "./transaction-event.hooks"
...
let hooks: TransactionEventHooks = {}
...
/**
 * Set hooks for TransactionEvent routes
 */
export function setTransactionEventHooks(h: TransactionEventHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["transactionId", "status", "message", "gatewayData", "createdAt"] as const
const RELATION_FIELDS = ["transaction"] as const
...
/**
 * Create TransactionEvent CRUD routes
 */
export function createTransactionEventRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[TransactionEvent] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.transactionEvent.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "TransactionEvent not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[TransactionEvent] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.transactionEvent.create({
        data: picked.data,
      })
      ...
      console.error("[TransactionEvent] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.transactionEvent.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[TransactionEvent] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.transactionEvent.delete({
        where: { id },
      })
      ...
      console.error("[TransactionEvent] Delete error:", error)
