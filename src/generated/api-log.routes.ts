...
import type { ApiLogHooks } from "./api-log.hooks"
...
/**
 * Set hooks for ApiLog routes
 */
export function setApiLogHooks(h: ApiLogHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "endpoint", "method", "statusCode", "ipAddress", "requestBody", "responseBody", "transactionId", "errorMessage", "createdAt"] as const
...
/**
 * Create ApiLog CRUD routes
 */
export function createApiLogRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ApiLog] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.apiLog.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ApiLog not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ApiLog] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.apiLog.create({
        data: picked.data,
      })
      ...
      console.error("[ApiLog] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.apiLog.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ApiLog] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ApiLog] Delete error:", error)
