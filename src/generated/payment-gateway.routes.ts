...
/**
 * Auto-generated PaymentGateway Routes
 *
...
import type { PaymentGatewayHooks } from "./payment-gateway.hooks"
...
/**
 * Set hooks for PaymentGateway routes
 */
export function setPaymentGatewayHooks(h: PaymentGatewayHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "displayName", "isEnabled", "environment", "credentials", "callbackUrl", "feeFixed", "feePercentage", "createdAt", "updatedAt"] as const
...
/**
 * Create PaymentGateway CRUD routes
 */
export function createPaymentGatewayRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[PaymentGateway] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.paymentGateway.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "PaymentGateway not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[PaymentGateway] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[PaymentGateway] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.paymentGateway.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[PaymentGateway] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[PaymentGateway] Delete error:", error)
