...
import type { AdminHooks } from "./admin.hooks"
...
/**
 * Set hooks for Admin routes
 */
export function setAdminHooks(h: AdminHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["email", "name", "passwordHash", "role", "lastLoginAt", "createdAt", "updatedAt"] as const
...
export function createAdminRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Admin not found" } }, 404)
      }
