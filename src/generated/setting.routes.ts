...
import type { SettingHooks } from "./setting.hooks"
...
/**
 * Set hooks for Setting routes
 */
export function setSettingHooks(h: SettingHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["key", "value", "group"] as const
...
/**
 * Create Setting CRUD routes
 */
export function createSettingRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[Setting] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Setting not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.setting.create({
        data: picked.data,
      })
